...
\pause
\begin{itemize}[<+-| alert@+>]
\item \textbf{Efficient Market Hypothesis} says that there is instant reaction
\item This may not be the case
\begin{itemize}
\pause
\item \emph{Behavioural finance} argues that investors' decisions diverge from \emph{expected decision theory}
