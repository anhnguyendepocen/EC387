...
\pause
\begin{itemize}[<+-| alert@+>]
\item \textbf{Efficient Market Hypothesis} suggests that there is instsant rection
...
\item Institutionl features can constrain investors' ability to \emph{arbitrage} diveregence from intrinsic value
\end{itemize}
\end{itemize}
\pause
...
\item There is a need to jump on the trend
\end{itemize}
\end{frame}
...
\begin{frame}{Event study tecnique 2}
...
\begin{itemize}[<+-| alert@+>]
\item Create a model of \emph{Expected return}
\begin{itemize}
\item Finout what is \emph{normal}
...
If there are confounders, this may bias the resutls
\end{block}
\end{frame}
