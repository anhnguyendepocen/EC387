...
\end{itemize}
\pause
The second is risky, post-crisis regulation make is increasingly costly
\end{frame}
...
\item Select a stock
\item Calculate the liquidity measure $L$ for that stock
\item How does liquidity perform around earning releases?
\end{itemize}
\end{frame}
